...
| `MarketStructureModule._find_swings` (`swing_strength` fractal rule) | Not source-stated | A bar is a swing high/low if it's strictly the most extreme high/low within `swing_strength` bars on each side (standard fractal/pivot convention; default 2 = "5-bar fractal"). | Added because BOS/CHoCH is used universally in the corpus but no document gives a swing-formation rule (doc 08 notes this gap explicitly). External, standard TA convention, not invented for this project specifically — documented per the project's rule. Validated live on both Binance BTCUSDT and Twelve Data EUR/USD across 1h/4h/1d without needing the kind of materiality-filter correction the SNR Zone module required — this concept doesn't have the same continuous-market blind spot. |
